...
function getTotalAmount() {
    ...
    $sql = "SELECT SUM(money) AS totalAmount FROM treasury_movement WHERE DATE(date) = CURDATE() AND Type = 'مدفوعات'";
...
$query = "SELECT id , photo, Type, Statement, sand, money, DATE_FORMAT(date, '%m/%d/%Y') AS formatted_date FROM treasury_movement WHERE DATE(date) = CURDATE() AND Type = 'مدفوعات'";
...
if (isset($_POST['save_form'])) {
    ...
    $photo = implode(",", $target_files);
        ...
        if (mysqli_stmt_execute($stmt)) {
            ...
            echo '<script>alert("حدث خطأ أثناء ادخال البيانات"); window.location.href = "back.php";</script>';
            exit;
        }
    }
...
if (isset($_POST['export_excel'])) {
    $query = "SELECT photo, Type, Statement, sand, money, DATE_FORMAT(date, '%m/%d/%Y') AS formatted_date FROM treasury_movement WHERE DATE(date) = CURDATE() AND Type = 'مدفوعات'";
    $result = mysqli_query($conn, $query);
    ...
    $filename = "تقرير حركه الخزينه مدفوعات.csv";
...
if (isset($_POST['search'])) {
    ...
    $query = "SELECT id , photo,  Type, Statement, sand, money, DATE_FORMAT(date, '%m-%d-%Y') AS formatted_date FROM treasury_movement WHERE DATE(date) = '$formatted_date' AND Type = 'مدفوعات' ";
    $result = mysqli_query($conn, $query);
...
if (isset($_POST['delete'])) {
    ...
    if (mysqli_stmt_execute($stmt)) {
        ...
        echo '<script>alert("حدث خطأ أثناء حذف الزيت"); window.location.href = "back.php";</script>';
        exit;
    }
}
}
...
<body>
    ...
    <input type="file" id="photo" class="save" name="photo[]" multiple>
        ...
        echo '</div>';       
                ...
                <a href=""><button  style="margin-bottom: 0px;
                    margin-top: 30px;
                    margin-left: 150px;" class="save">اضافه/حدف</button></a>
                    <div>
...
const printWindow = window.open(`back_generate_report.php?search_date=${searchDate}`, '_blank', 'width=800,height=600');
