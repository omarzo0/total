...
if (is_logged_in() || $_SESSION['id'] == 1) {
    ...
    $query = "SELECT id , photo, sand, money, DATE_FORMAT(date, '%m/%d/%Y') AS formatted_date
              FROM expenses 
              WHERE 1 $whereClause";
...
echo '<div class="header__item"><a id="name" class="filter__link" href="#"> المبلغ</a></div>';
echo '<div class="header__item"><a id="wins" class="filter__link filter__link--number" href="#"> السند</a></div>';
echo '</div>';       
