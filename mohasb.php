...
$query = "SELECT photo, id, name, sand, money, DATE_FORMAT(date, '%m/%d/%Y') AS formatted_date FROM hsabat WHERE DATE(date) = CURDATE() AND name = 'المحاسب'";
...
if (isset($_POST['save_form'])) {
  ...
  $photo = implode(",", $target_files);
        ...
        if (mysqli_stmt_execute($stmt)) {
            ...
            unset($sand);
            unset($money);
            header('Location: mohasb.php');
            exit;
        } else {
            echo '<script>alert("حدث خطأ أثناء ادخال البيانات"); window.location.href = "mohasb.php";</script>';
            exit;
        }
    }
...
if (isset($_POST['export_excel'])) {
    $query = "SELECT photo, name, sand, money, DATE_FORMAT(date, '%m/%d/%Y') AS formatted_date FROM hsabat WHERE DATE(date) = CURDATE() AND name = 'المحاسب'";
    $result = mysqli_query($conn, $query);
...
if (isset($_POST['search'])) {
    ...
    $query = "SELECT photo, id, name, sand, money, DATE_FORMAT(date, '%m-%d-%Y') AS formatted_date FROM hsabat WHERE DATE(date) = '$formatted_date' AND name = 'المحاسب'";
    $result = mysqli_query($conn, $query);
...
if (isset($_POST['delete'])) {
    ...
    if (mysqli_stmt_execute($stmt)) {
        ...
        echo '<script>alert("حدث خطأ أثناء حذف الزيت"); window.location.href = "mohasb.php";</script>';
        exit;
    }
}
